...
LiteRtStatus LiteRtSamsungInitialize(
  ...
  if (runtime_context->get_environment_options(environment, &environment_options) ==
      kLiteRtStatusOk) {
...
LiteRtStatus LiteRtSamsungDeviceContextCreate(
  ...
  if (auto context =
          LiteRtDispatchDeviceContextT::Create(runtime_context, static_enn_manager.get());
      context) {
...
const LiteRtDispatchInterface_V1 TheInterface = {
    /*.abi_header=*/{
