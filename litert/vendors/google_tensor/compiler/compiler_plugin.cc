...
LiteRtStatus PopulateCompilerConfig(
    LiteRtCompilerPlugin compiler_plugin, absl::string_view soc_model,
    GoogleTensorOptions& google_tensor_options) {
  LITERT_ASSIGN_OR_RETURN(const litert::google_tensor::Adapter* const adapter,
                          compiler_plugin->GetAdapter());

  GoogleTensorCompilerConfig* const compiler_config =
  ...
  const LiteRtApiVersion litert_version = compiler_plugin->GetLiteRtVersion();
  const std::string api_version_str =
  ...
  if (adapter->IsAot()) {
    const std::string valid_soc_model = [soc_model]() -> std::string {
      absl::string_view soc_model_view =
          soc_model.empty() ? "Tensor_G3" : soc_model;
      if (soc_model_view == "g5" || soc_model_view == "g4" ||
          soc_model_view == "g3") {
        LITERT_LOG(
            LITERT_WARNING,
            "g3/g4/g5 is deprecated. Please use Tensor_G3/G4/G5 instead.");
        return absl::StrCat("Tensor_", absl::AsciiStrToUpper(soc_model_view));
      }
      return std::string(soc_model_view);
    }();
    // Set device type.
    DeviceType device_type;
...
int32_t GetOrInsertOpCode(
  ...
  auto new_code = std::make_unique<tflite::OperatorCodeT>();
...
LiteRtStatus LiteRtCompilerPluginCompile(
  ...
  const absl::string_view buffer_str(reinterpret_cast<const char*>(buf.Data()),
                                     buf.Size());
  ...
  const absl::string_view model_buffer_view(buffer_str);
  ...
  const auto compile_status = adapter->Compile(
...
static const LiteRtCompilerPluginInterface_V1
    GoogleTensorCompilerPluginInterface = {
        .abi_header =
            {
