...
inline LiteRtStatus MockDispatchInvocationContextCreate(
  ...
  auto* dev_ctx =
      reinterpret_cast<MockDispatchDeviceContextT*>(device_context);
...
inline LiteRtStatus MockDispatchAttachInput(
    ...
    LiteRtTensorBufferHandle tensor_buffer_handle) {
  auto* ctx = reinterpret_cast<MockDispatchInvocationContextT*>(
      invocation_context);
  if (!ctx) return kLiteRtStatusErrorInvalidArgument;
...
typedef LiteRtStatus(LITERT_API_CALL* MockDispatchFutureExtensionT)();

inline LiteRtStatus LITERT_API_CALL MockDispatchFutureExtension() {
  return kLiteRtStatusOk;
}
