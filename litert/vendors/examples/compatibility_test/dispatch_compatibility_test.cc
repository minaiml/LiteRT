...
class DispatchCompatibilityTest : public ::testing::Test {
 protected:
  ...
  LiteRtStatus InitDispatch(absl::string_view subdir,
    ...
    std::string src = Join(
        {GetLiteRtPath("vendors/examples/compatibility_test"), so_name});
...
TEST_F(DispatchCompatibilityTest, Scenario1a_OlderVendor_SupportedInterface) {
  LITERT_ASSERT_OK(InitDispatch("older_supported",
                               "libLiteRtDispatch_OlderSupported.so"));
...
TEST_F(DispatchCompatibilityTest, Scenario2a_NewerVendor_SupportedInterface) {
  LITERT_ASSERT_OK(InitDispatch("newer_supported",
                               "libLiteRtDispatch_NewerSupported.so"));
...
TEST_F(DispatchCompatibilityTest, Scenario3a_SameVersion_SupportedInterface) {
  LITERT_ASSERT_OK(InitDispatch("same_supported",
                               "libLiteRtDispatch_SameSupported.so"));
...
TEST_F(DispatchCompatibilityTest,
  ...
  LiteRtStatus status = InitDispatch("null_interface",
                                    "libLiteRtDispatch_NullInterface.so");
  EXPECT_EQ(status, kLiteRtStatusErrorWrongVersion);
}
...
TEST_F(DispatchCompatibilityTest, Scenario4a_IncompatibleMajorVersionRejected) {
  LiteRtStatus status = InitDispatch("incompatible_major",
                                    "libLiteRtDispatch_Incompatible.so");
  EXPECT_EQ(status, kLiteRtStatusErrorWrongVersion);
}
...
TEST_F(DispatchCompatibilityTest, Scenario4b_CorruptedHeaderRejected) {
  LiteRtStatus status = InitDispatch("corrupted_header",
                                    "libLiteRtDispatch_CorruptedHeader.so");
  EXPECT_EQ(status, kLiteRtStatusErrorWrongVersion);
}
