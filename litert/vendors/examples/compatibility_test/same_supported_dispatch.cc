...
LiteRtStatus LITERT_API_CALL GetSameVendorId(const char** vendor_id) {
  if (!vendor_id) return kLiteRtStatusErrorInvalidArgument;
  *vendor_id = "SameSupportedDispatchVendor";
  return kLiteRtStatusOk;
}

static const LiteRtDispatchInterface_V1 kSameSupportedDispatchInterface = {
    .abi_header =
        {
  ...
  if (litert_runtime_version.major == 1) {
    if (interface_id == kLiteRtInterfaceBasic) {
      *out_interface = const_cast<LiteRtDispatchInterface_V1*>(
          &kSameSupportedDispatchInterface);
      return kLiteRtStatusOk;
    }
  }
