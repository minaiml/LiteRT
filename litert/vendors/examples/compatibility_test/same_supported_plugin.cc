...
const char* GetSameSocManufacturer() { return "SameSupportedManufacturer"; }

static const LiteRtCompilerPluginInterface_V1_1 kSameSupportedInterface = {
    .abi_header =
        {
    ...
    .get_compiler_plugin_soc_manufacturer = GetSameSocManufacturer,
...
LiteRtCompilerPluginQueryInterface(LiteRtCompilerPluginInterfaceId interface_id,
  ...
  if (litert_runtime_version.major == 1) {
    if (interface_id == kLiteRtCompilerPluginInterfaceBasic) {
      *out_interface = const_cast<LiteRtCompilerPluginInterface_V1_1*>(
          &kSameSupportedInterface);
