...
// Calling convention for LiteRT C API functions and function pointer callbacks.
// On 32-bit Windows/x86 targets, defaults to __cdecl to ensure compatibility
// across different compiler default flags (e.g. /Gz __stdcall).
#if defined(_WIN32) || defined(LITERT_WINDOWS_OS)
#define LITERT_API_CALL __cdecl
#elif defined(__i386__)
#define LITERT_API_CALL __attribute__((cdecl))
#else
#define LITERT_API_CALL
#endif
...
typedef enum LiteRtHwAccelerators {
  ...
  // as the underlying type for Rust `bindgen` compatibility and under
  // -fshort-enums.
  _kLiteRtHwAcceleratorNegativeDummy = -1,
  _kLiteRtHwAcceleratorForceInt32 = 0x7fffffff,
} LiteRtHwAccelerators;

LITERT_ABI_STATIC_ASSERT(sizeof(LiteRtHwAccelerators) == 4,
                         "LiteRtHwAccelerators size must be 4 bytes");
